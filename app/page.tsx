...
const TH = {
  dark:  { bg:"#0F0F14", surface:"#1A1A24", card:"#1E1E2E", cb:"#2D2D42", rd:"#252535", tx:"#F0F0FF", ts:"#9090B0", tm:"#55557A", pl:"#252538", pb:"#3A3A55", ib:"#252538", ibr:"#3A3A55", dh:"#16161F", nb:"#141420", nbr:"#2D2D42", st:"#6060A0", cs:"0 2px 12px rgba(0,0,0,0.3)" },
  light: { bg:"#F0F0FA", surface:"#FFFFFF",  card:"#FFFFFF",  cb:"#E0E0F0", rd:"#EBEBF8", tx:"#111128", ts:"#5555AA", tm:"#9999CC", pl:"#F0F0FF", pb:"#CCCCEE", ib:"#F5F5FF", ibr:"#CCCCEE", dh:"#F5F5FF", nb:"#FFFFFF",  nbr:"#E0E0F0", st:"#8888CC", cs:"0 2px 8px rgba(100,100,200,0.08)" },
};

function tod(o=0):string { const d=new Date(); d.setDate(d.getDate()+o); return d.toISOString().split("T")[0]; }
function fmtCur(n:number,c="MYR"):string { try { return new Intl.NumberFormat("en-MY",{style:"currency",currency:c,minimumFractionDigits:2}).format(n); } catch { return `${c} ${n.toFixed(2)}`; } }
...
export default function Root() {
  ...
  async function doSignup() {
    ...
    const {error:suErr} = await sb.auth.signUp({
      email: sf.email.toLowerCase().trim(),
      password: sf.password,
      options: { emailRedirectTo:undefined, data:{display_name:sf.name.trim()} },
    });
    if(suErr){ setBusy(false); return setErr(suErr.message); }
    const {data:siData,error:siErr} = await sb.auth.signInWithPassword({
      email: sf.email.toLowerCase().trim(),
      password: sf.password,
    });
    setBusy(false);
    if(siErr) return setErr(siErr.message);
    const u=siData.user!;
    setUser({id:u.id,name:sf.name.trim(),email:u.email,color:MCOLORS[0]});
    setScr(S.LOBBY);
  }
  ...
  async function doSignin() {
    ...
    const {data,error} = await sb.auth.signInWithPassword({
      email: lf.email.toLowerCase().trim(),
  ...
  async function doCreate() {
    ...
    if(!session){ setBusy(false); return setErr("Session expired — please sign in again"); }
    const uid = session.user.id;
    const code = mkCode();
    const {data:rm,error:re} = await sb.from("rooms").insert({name:rname.trim(),share_code:code,created_by:uid}).select().single();
    if(re){ setBusy(false); return setErr(re.message); }
    await sb.from("room_members").insert({room_id:rm.id,user_id:uid,display_name:user.name,color:MCOLORS[0]});
    await sb.from("categories").insert(DEFAULT_CATS.map(c=>({...c,room_id:rm.id,created_by:uid})));
    keyRef.current = await deriveRoomKey(code);
    const {data:cats} = await sb.from("categories").select("*").eq("room_id",rm.id);
    setRoom({...rm,members:[{id:uid,name:user.name,color:MCOLORS[0]}],categories:cats||[],transactions:[]});
    setBusy(false);
    setScr(S.APP);
  }
  ...
  async function doJoin() {
    ...
    const [rm,mems,cats,txnsRaw] = await Promise.all([
      ...
      sb.from("room_members").select("*").eq("room_id",roomId),
      sb.from("categories").select("*").eq("room_id",roomId),
      sb.from("transactions").select("*").eq("room_id",roomId).order("date_bucket",{ascending:false}).limit(200),
    ]);
    const txns = await Promise.all((txnsRaw.data||[]).map(async(t:any)=>{
      try { const p=await decrypt<any>(t.payload,t.iv,keyRef.current!); return {...p,id:t.id,userId:t.user_id,categoryId:t.category_id,decrypted:true}; }
      catch { return {id:t.id,userId:t.user_id,categoryId:t.category_id,amount:0,decrypted:false}; }
    }));
    setRoom({...rm.data,members:(mems.data||[]).map((m:any)=>({id:m.user_id,name:m.display_name,color:m.color})),categories:cats.data||[],transactions:txns});
    setBusy(false);
    setScr(S.APP);
  }
  ...
  function ErrBox(){ return err?<div style={{background:"#FF6B6B20",border:"1px solid #FF6B6B40",borderRadius:10,padding:"10px 14px",fontSize:13,color:"#FF6B6B"}}>⚠️ {err}</div>:null; }
    ...
    <div style={{minHeight:"100vh",background:T.bg,display:"flex",flexDirection:"column"}}>
      ...
      <div style={{padding:"28px 24px",display:"flex",flexDirection:"column",gap:14}}>
        {[{label:"Your name",key:"name",type:"text",ph:"e.g. Clement"},{label:"Email",key:"email",type:"email",ph:"you@email.com"},{label:"Password",key:"password",type:"password",ph:"Min. 6 characters"},{label:"Confirm password",key:"confirm",type:"password",ph:"Repeat password"}].map(f=>(
          <div key={f.key}>
            <div style={{fontSize:12,color:T.ts,marginBottom:6,fontWeight:600}}>{f.label}</div>
            <input type={f.type} placeholder={f.ph} value={(sf as any)[f.key]} onChange={e=>setSf(p=>({...p,[f.key]:e.target.value}))} style={inp}/>
          </div>
        ))}
        <ErrBox/>
    ...
    <div style={{minHeight:"100vh",background:T.bg,display:"flex",flexDirection:"column"}}>
      ...
      <div style={{padding:"28px 24px",display:"flex",flexDirection:"column",gap:14}}>
        {[{label:"Email",key:"email",type:"email",ph:"you@email.com"},{label:"Password",key:"password",type:"password",ph:"Your password"}].map(f=>(
          <div key={f.key}>
            <div style={{fontSize:12,color:T.ts,marginBottom:6,fontWeight:600}}>{f.label}</div>
            <input type={f.type} placeholder={f.ph} value={(lf as any)[f.key]} onChange={e=>setLf(p=>({...p,[f.key]:e.target.value}))} style={inp}/>
          </div>
        ))}
        <ErrBox/>
  ...
  if(scr===S.APP&&room&&user) return <MainApp dark={dark} setDark={setDark} user={user} room={room} setRoom={setRoom} cryptoKey={keyRef.current} onSignOut={doSignOut} onCopyCode={copyCode} copied={copied} T={T}/>;
  return null;
}

function MainApp({dark,setDark,user,room,setRoom,cryptoKey,onSignOut,onCopyCode,copied,T}:any) {
    ...
    const ch = sb.channel(`room:${room.id}`)
      .on("postgres_changes",{event:"INSERT",schema:"public",table:"transactions",filter:`room_id=eq.${room.id}`},async(p:any)=>{
        ...
        try {
          ...
          setRoom((r:any)=>({...r,transactions:[t,...r.transactions.filter((x:any)=>x.id!==t.id)]}));
        } catch {}
      })
  ...
  async function addTxn() {
    ...
    const catId = form.catId||cats[0]?.id;
    await sb.from("transactions").insert({room_id:room.id,user_id:user.id,category_id:catId,payload:enc,iv,date_bucket:form.date});
  ...
  const filtered = useMemo(()=>[...txns].filter((t:any)=>who==="all"||t.userId===user.id).sort((a:any,b:any)=>(b.date||"").localeCompare(a.date||"")),[txns,who,user.id]);
  ...
  const weeks    = useMemo(()=>{ const m:any={}; filtered.forEach((t:any)=>{ const w=wkOf(t.date||tod()); m[w]=(m[w]||0)+(t.amount||0); }); return Object.keys(m).sort().slice(-5).map(k=>({week:k,total:m[k]})); },[filtered]);
  const maxWeek  = Math.max(...weeks.map((w:any)=>w.total),1);
  const byDay    = useMemo(()=>{ const m:any={}; filtered.forEach((t:any)=>{ const d=t.date||tod(); m[d]=(m[d]||0)+(t.amount||0); }); return Object.entries(m).sort(([,a],[,b])=>(b as number)-(a as number)).slice(0,3).map(([d,v])=>({date:d,total:v as number})); },[filtered]);
  const grouped  = useMemo(()=>{ const g:any={}; filtered.forEach((t:any)=>{ const d=t.date||tod(); if(!g[d])g[d]=[]; g[d].push(t); }); return g; },[filtered]);
  ...
  function TR({t,last}:any){
    ...
    return <div style={{display:"flex",alignItems:"center",gap:12,padding:"13px 16px",borderBottom:last?"none":`1px solid ${T.rd}`}}>
      ...
      <div style={{flex:1,minWidth:0}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6,marginTop:3}}><Av u={u} sz={14}/><span style={{fontSize:11,color:T.ts}}>{u.name} · {relDate(t.date||tod())}</span></div>
      </div>
      <div style={{textAlign:"right"}}>
        <div style={{fontSize:14,fontWeight:700,color:T.tx}}>{fmtCur(t.amount||0,t.currency||"MYR")}</div>
  ...
  function Sheet({show,onClose,title,children}:any){
    if(!show) return null;
    return <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",display:"flex",alignItems:"flex-end",justifyContent:"center",zIndex:200}} onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={{background:T.card,borderRadius:"22px 22px 0 0",width:"100%",maxWidth:430,padding:"0 20px 36px",maxHeight:"90vh",overflowY:"auto",border:`1px solid ${T.cb}`}}>
        <div style={{width:40,height:4,background:T.pb,borderRadius:2,margin:"14px auto 22px"}}/>
        <div style={{fontSize:20,fontWeight:800,marginBottom:20,color:T.tx}}>{title}</div>
        {children}
      </div>
    </div>;
  }
  ...
  if(drill&&drillInfo) return (
    <div style={{fontFamily:"system-ui,sans-serif",maxWidth:430,margin:"0 auto",paddingBottom:40,minHeight:"100vh",background:T.bg}}>
      <div style={{background:G,padding:"20px 20px 48px"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:14}}>
          <div style={{width:56,height:56,borderRadius:16,background:"#ffffff20",display:"flex",alignItems:"center",justifyContent:"center",fontSize:28}}>{drillInfo.icon}</div>
  ...
  return (
    <div style={{fontFamily:"system-ui,sans-serif",maxWidth:430,margin:"0 auto",paddingBottom:90,minHeight:"100vh",background:T.bg}}>
      ...
      {tab==="dashboard"&&<>
        <div style={{background:G,padding:"24px 20px 64px"}}>
          ...
          <div style={{display:"flex",gap:8,marginTop:14,overflowX:"auto"}}>
            {[{id:"all",name:"Everyone"},...mems].map((u:any)=><button key={u.id} onClick={()=>setWho(u.id)} style={{padding:"6px 14px",borderRadius:99,border:`1.5px solid ${who===u.id?"#fff":"#ffffff40"}`,background:who===u.id?"#ffffff25":"#ffffff12",color:"#fff",fontSize:12,cursor:"pointer",fontWeight:who===u.id?700:400,whiteSpace:"nowrap",flexShrink:0}}>{u.name}</button>)}
          </div>
        </div>

        <div style={{margin:"-30px 16px 20px",display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,position:"relative",zIndex:10}}>
          {[{label:"Biggest day",val:byDay[0]?fmtCur(byDay[0].total):"—",sub:byDay[0]?relDate(byDay[0].date):"no data",grad:"linear-gradient(135deg,#FF6B6B,#FF8E53)"},{label:"Transactions",val:filtered.length,sub:"this period",grad:"linear-gradient(135deg,#4ECDC4,#45B7D1)"}].map((m,i)=>(
            <div key={i} style={{borderRadius:16,padding:16,background:m.grad,boxShadow:"0 4px 16px rgba(0,0,0,0.2)"}}>
              <div style={{fontSize:10,color:"#ffffff99",marginBottom:6,fontWeight:700,textTransform:"uppercase"}}>{m.label}</div>
        ...
        {weeks.length>0&&<Sec title="Weekly trend">
          <div style={{background:T.card,borderRadius:16,border:`1px solid ${T.cb}`,padding:"16px 16px 12px",boxShadow:T.cs}}>
            <div style={{display:"flex",alignItems:"flex-end",gap:8,height:90,justifyContent:"space-between",borderBottom:`1px solid ${T.rd}`,paddingBottom:10,marginBottom:10}}>
              {weeks.map((w:any,i:number)=>{ const h=Math.max(8,Math.round((w.total/maxWeek)*80)); const isL=i===weeks.length-1; return <div key={w.week} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:5}}><div style={{fontSize:10,fontWeight:isL?700:400,color:isL?"#6C63FF":T.ts}}>{fmtCur(w.total).replace(/[^0-9.,]/g,"").replace(/\.00$/,"")}</div><div style={{width:"100%",height:h,borderRadius:"6px 6px 4px 4px",background:isL?"linear-gradient(180deg,#6C63FF,#9B59B6)":T.surface,border:`1px solid ${isL?"#6C63FF60":T.cb}`}}/></div>; })}
            </div>
            <div style={{display:"flex",gap:8,justifyContent:"space-between"}}>
              {weeks.map((w:any,i:number)=>{ const s=new Date(w.week),e=new Date(w.week); e.setDate(e.getDate()+6); const f2=(d:Date)=>d.toLocaleDateString("en-MY",{day:"numeric",month:"short"}); const isL=i===weeks.length-1; return <div key={w.week} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:1}}><div style={{fontSize:9,color:isL?"#6C63FF":T.ts,fontWeight:isL?700:400,textAlign:"center",lineHeight:1.3}}>{f2(s)}</div><div style={{fontSize:8,color:T.tm,textAlign:"center"}}>– {f2(e)}</div></div>; })}
            </div>
          </div>
        </Sec>}

        {byDay.length>0&&<Sec title="Highest spending days"><Card>
          {byDay.map((d:any,i:number)=>(
            <div key={d.date} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 16px",borderBottom:i<byDay.length-1?`1px solid ${T.rd}`:"none"}}>
              <div style={{display:"flex",alignItems:"center",gap:12}}>
                <div style={{width:36,height:36,borderRadius:10,background:["#FF6B6B25","#F0A50025","#4ECDC425"][i],display:"flex",alignItems:"center",justifyContent:"center",fontSize:16}}>{["🔥","⚡","💸"][i]}</div>
                <div><div style={{fontSize:14,fontWeight:600,color:T.tx}}>{relDate(d.date)}</div><div style={{fontSize:11,color:T.ts}}>{txns.filter((t:any)=>t.date===d.date).length} transactions</div></div>
              </div>
              <div style={{fontSize:16,fontWeight:800,color:["#FF6B6B","#F0A500","#4ECDC4"][i]}}>{fmtCur(d.total)}</div>
            </div>
          ))}
        </Card></Sec>}

        <Sec title="Spending by category"><Card>
          {byCat.length===0?<div style={{padding:40,textAlign:"center",color:T.ts}}>No spending yet — add your first expense!</div>:
          byCat.map((c:any,i:number)=>(
            <div key={c.id} onClick={()=>setDrill(c.id)} style={{display:"flex",alignItems:"center",gap:12,padding:"14px 16px",borderBottom:i<byCat.length-1?`1px solid ${T.rd}`:"none",cursor:"pointer"}}>
              ...
              <div style={{flex:1,minWidth:0}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}><div style={{fontSize:14,fontWeight:600,color:T.tx}}>{c.label}</div><div style={{fontSize:14,fontWeight:800,color:c.color}}>{fmtCur(c.total)}</div></div>
                <div style={{height:6,background:T.rd,borderRadius:3}}><div style={{width:`${Math.min(100,(c.total/maxCat)*100)}%`,height:"100%",background:`linear-gradient(90deg,${c.color},${c.color}88)`,borderRadius:3}}/></div>
                <div style={{fontSize:10,color:T.ts,marginTop:4}}>{c.count} transaction{c.count!==1?"s":""} · {total>0?Math.round((c.total/total)*100):0}%</div>
              </div>
              <div style={{color:T.tm,fontSize:18}}>›</div>
            </div>
          ))}
        </Card></Sec>

        <Sec title="Recent transactions" action={<button onClick={()=>setTab("transactions")} style={{fontSize:12,color:"#6C63FF",background:"none",border:"none",cursor:"pointer",fontWeight:700}}>See all →</button>}><Card>
          {filtered.length===0?<div style={{padding:40,textAlign:"center",color:T.ts}}>No transactions yet</div>:filtered.slice(0,4).map((t:any,i:number)=><TR key={t.id} t={t} last={i===3||i===filtered.slice(0,4).length-1}/>)}
        </Card></Sec>
      </>}

      {tab==="transactions"&&<>
        <div style={{background:G,padding:"24px 20px 28px"}}>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}><div style={{fontSize:20,fontWeight:800,color:"#fff"}}>Transactions</div><button onClick={()=>setDark((d:boolean)=>!d)} style={{background:"#ffffff20",border:"1px solid #ffffff30",borderRadius:10,padding:"6px 10px",fontSize:16,cursor:"pointer"}}>{dark?"☀️":"🌙"}</button></div>
          <div style={{fontSize:13,color:"#ffffffAA"}}>{filtered.length} total · {fmtCur(total)}</div>
        </div>
        <div style={{margin:"14px 16px"}}>
          <div style={{display:"flex",gap:8,overflowX:"auto",paddingBottom:14}}>
            {cats.filter((c:any)=>filtered.some((t:any)=>t.categoryId===c.id)).map((c:any)=><button key={c.id} onClick={()=>setDrill(c.id)} style={{padding:"7px 12px",borderRadius:99,border:`1.5px solid ${c.color}50`,background:c.color+"18",color:c.color,fontSize:12,cursor:"pointer",fontWeight:600,whiteSpace:"nowrap",flexShrink:0}}>{c.icon} {c.label}</button>)}
          </div>
          <Card>
            {Object.entries(grouped).sort(([a],[b])=>b.localeCompare(a)).map(([date,dt]:any)=>(
              <div key={date}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 16px",background:T.dh,borderBottom:`1px solid ${T.rd}`}}>
                  <span style={{fontSize:12,fontWeight:700,color:T.ts,textTransform:"uppercase"}}>{relDate(date)}</span>
                  <span style={{fontSize:13,fontWeight:800,color:T.tx}}>{fmtCur(dt.reduce((s:number,t:any)=>s+(t.amount||0),0))}</span>
                </div>
                {dt.map((t:any,i:number)=><TR key={t.id} t={t} last={i===dt.length-1}/>)}
              </div>
            ))}
            {filtered.length===0&&<div style={{padding:40,textAlign:"center",color:T.ts}}>No transactions yet</div>}
          </Card>
        </div>
      </>}
      ...
      {tab==="analysis"&&<>
        <div style={{background:G,padding:"24px 20px 28px"}}>
          <div style={{display:"flex",justifyContent:"space-between"}}><div><div style={{fontSize:20,fontWeight:800,color:"#fff"}}>Analysis</div><div style={{fontSize:13,color:"#ffffffAA",marginTop:2}}>Your spending insights</div></div><button onClick={()=>setDark((d:boolean)=>!d)} style={{background:"#ffffff20",border:"1px solid #ffffff30",borderRadius:10,padding:"6px 10px",fontSize:16,cursor:"pointer"}}>{dark?"☀️":"🌙"}</button></div>
        </div>
        <div style={{margin:"14px 16px"}}>
          <Sec title="Household split">
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
              {byUser.map((u:any)=>{ const pct=gTotal>0?Math.round((u.total/gTotal)*100):0; return <div key={u.id} style={{borderRadius:16,padding:16,background:u.color+"18",border:`1px solid ${u.color}30`}}><Av u={u} sz={36}/><div style={{fontSize:13,fontWeight:700,color:T.tx,marginTop:10}}>{u.name}</div><div style={{fontSize:22,fontWeight:800,color:u.color,marginTop:4}}>{fmtCur(u.total)}</div><div style={{fontSize:11,color:T.ts,marginTop:2}}>{pct}% of total</div><div style={{height:5,background:T.rd,borderRadius:3,marginTop:10}}><div style={{width:`${pct}%`,height:"100%",background:u.color,borderRadius:3}}/></div></div>; })}
            </div>
          </Sec>
          <Sec title="Category breakdown"><Card>
            {byCat.length===0?<div style={{padding:32,textAlign:"center",color:T.ts}}>No data yet</div>:
            byCat.slice(0,7).map((c:any,i:number)=>(
              <div key={c.id} onClick={()=>setDrill(c.id)} style={{display:"flex",alignItems:"center",gap:10,padding:"13px 16px",borderBottom:i<Math.min(byCat.length,7)-1?`1px solid ${T.rd}`:"none",cursor:"pointer"}}>
                <div style={{width:34,height:34,borderRadius:10,background:c.color+"20",display:"flex",alignItems:"center",justifyContent:"center",fontSize:17,flexShrink:0}}>{c.icon}</div>
                <div style={{flex:1}}><div style={{display:"flex",justifyContent:"space-between",fontSize:13,marginBottom:5}}><span style={{fontWeight:600,color:T.tx}}>{c.label}</span><span style={{color:T.ts}}>{total>0?Math.round((c.total/total)*100):0}%</span></div><div style={{height:7,background:T.rd,borderRadius:4}}><div style={{width:`${total>0?(c.total/total)*100:0}%`,height:"100%",background:`linear-gradient(90deg,${c.color},${c.color}77)`,borderRadius:4}}/></div></div>
                <div style={{fontSize:13,fontWeight:800,color:c.color,minWidth:68,textAlign:"right"}}>{fmtCur(c.total)}</div>
              </div>
            ))}
          </Card></Sec>
          <Sec title="Averages">
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
              {[{label:"Daily avg",val:fmtCur(total/Math.max(1,Object.keys(grouped).length)),color:"#6C63FF",grad:"linear-gradient(135deg,#6C63FF22,#9B59B622)"},{label:"Per transaction",val:fmtCur(filtered.length>0?total/filtered.length:0),color:"#FF6B9D",grad:"linear-gradient(135deg,#FF6B9D22,#C44DFF22)"}].map((m,i)=>(
                <div key={i} style={{borderRadius:14,padding:14,background:m.grad,border:`1px solid ${m.color}30`}}><div style={{fontSize:10,color:T.ts,marginBottom:6,fontWeight:700,textTransform:"uppercase"}}>{m.label}</div><div style={{fontSize:18,fontWeight:800,color:m.color}}>{m.val}</div></div>
              ))}
            </div>
          </Sec>
        </div>
      ...
      {tab==="categories"&&<>
        <div style={{background:G,padding:"24px 20px 28px"}}>
          <div style={{display:"flex",justifyContent:"space-between"}}><div><div style={{fontSize:20,fontWeight:800,color:"#fff"}}>Categories</div><div style={{fontSize:13,color:"#ffffffAA",marginTop:2}}>{cats.length} categories</div></div><button onClick={()=>setDark((d:boolean)=>!d)} style={{background:"#ffffff20",border:"1px solid #ffffff30",borderRadius:10,padding:"6px 10px",fontSize:16,cursor:"pointer"}}>{dark?"☀️":"🌙"}</button></div>
        </div>
        <div style={{margin:"14px 16px"}}>
          <Card>
            {cats.map((c:any,i:number)=>{ const ct=filtered.filter((t:any)=>t.categoryId===c.id).reduce((s:number,t:any)=>s+(t.amount||0),0); const cc=filtered.filter((t:any)=>t.categoryId===c.id).length; return <div key={c.id} onClick={()=>setDrill(c.id)} style={{display:"flex",alignItems:"center",gap:14,padding:"14px 16px",borderBottom:i<cats.length-1?`1px solid ${T.rd}`:"none",cursor:"pointer"}}><div style={{width:46,height:46,borderRadius:14,background:c.color+"22",display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,border:`1px solid ${c.color}40`,flexShrink:0}}>{c.icon}</div><div style={{flex:1}}><div style={{fontSize:14,fontWeight:700,color:T.tx}}>{c.label}</div><div style={{fontSize:11,color:T.ts,marginTop:2}}>{cc} transaction{cc!==1?"s":""}</div></div><div style={{textAlign:"right"}}><div style={{fontSize:15,fontWeight:800,color:ct>0?c.color:T.tm}}>{ct>0?fmtCur(ct):"—"}</div></div></div>; })}
          </Card>
      ...
      {tab==="profile"&&<>
        ...
        <div style={{margin:"20px 16px",display:"flex",flexDirection:"column",gap:14}}>
          <div style={{background:T.card,borderRadius:16,border:`1px solid ${T.cb}`,padding:20,display:"flex",alignItems:"center",gap:16}}>
            <div style={{width:60,height:60,borderRadius:"50%",background:"#6C63FF30",color:"#6C63FF",display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,fontWeight:800}}>{inits(user.name)}</div>
            <div><div style={{fontSize:18,fontWeight:800,color:T.tx}}>{user.name}</div><div style={{fontSize:13,color:T.ts,marginTop:2}}>{user.email}</div></div>
          </div>
      ...
      <div style={{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:430,background:T.nb,borderTop:`1px solid ${T.nbr}`,display:"flex",zIndex:100}}>
        {[{id:"dashboard",icon:"⬛",label:"Home"},{id:"transactions",icon:"☰",label:"Spending"},{id:"analysis",icon:"📊",label:"Analysis"},{id:"categories",icon:"🏷",label:"Categories"},{id:"profile",icon:"👤",label:"Profile"}].map(n=>(
          <button key={n.id} onClick={()=>setTab(n.id)} style={{flex:1,padding:"11px 0 9px",border:"none",background:"none",cursor:"pointer",fontSize:10,color:tab===n.id?"#6C63FF":T.ts,fontWeight:tab===n.id?700:400,display:"flex",flexDirection:"column",alignItems:"center",gap:3}}>
      ...
      <Sheet show={showInv} onClose={()=>setShowInv(false)} title={`Invite to ${room.name}`}>
        ...
        <div style={{background:T.surface,borderRadius:14,padding:20,border:`1px dashed ${T.cb}`,textAlign:"center",marginBottom:20}}>
          <div style={{fontSize:11,color:T.ts,marginBottom:8,fontWeight:600,textTransform:"uppercase"}}>Room share code</div>
          <div style={{fontSize:44,fontWeight:900,color:"#6C63FF",letterSpacing:"10px"}}>{room.share_code}</div>
        </div>
      ...
      <Sheet show={showAdd} onClose={()=>setShowAdd(false)} title="Add expense">
        <div style={{marginBottom:14}}><div style={{fontSize:12,color:T.ts,marginBottom:6,fontWeight:600}}>Amount</div>
          <div style={{display:"flex",gap:8}}>
            <select value={form.currency} onChange={e=>setForm(f=>({...f,currency:e.target.value}))} style={{...inp,width:90,flex:"0 0 90px"}}>{CURRS.map(c=><option key={c}>{c}</option>)}</select>
            <input type="number" placeholder="0.00" value={form.amount} onChange={e=>setForm(f=>({...f,amount:e.target.value}))} style={{...inp,flex:1,fontSize:20,fontWeight:700}} autoFocus/>
          </div>
        </div>
        ...
        <div style={{marginBottom:14}}><div style={{fontSize:12,color:T.ts,marginBottom:8,fontWeight:600}}>Category</div>
          <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
            {cats.map((c:any)=><button key={c.id} onClick={()=>setForm(f=>({...f,catId:c.id}))} style={{padding:"7px 12px",borderRadius:20,border:`1.5px solid ${form.catId===c.id?c.color:T.pb}`,background:form.catId===c.id?c.color+"22":T.pl,color:form.catId===c.id?c.color:T.ts,fontSize:12,cursor:"pointer",fontWeight:form.catId===c.id?700:400}}>{c.icon} {c.label}</button>)}
          </div>
        </div>
        <div style={{marginBottom:14}}><div style={{fontSize:12,color:T.ts,marginBottom:6,fontWeight:600}}>Date</div><input type="date" value={form.date} onChange={e=>setForm(f=>({...f,date:e.target.value}))} style={inp}/></div>
        <button onClick={addTxn} disabled={busy} style={{width:"100%",padding:"15px",background:G,color:"#fff",border:"none",borderRadius:14,fontSize:16,fontWeight:800,cursor:"pointer",marginTop:6,opacity:busy?0.7:1}}>{busy?"Saving…":"Add expense"}</button>
      </Sheet>
      ...
      <Sheet show={showCat} onClose={()=>setShowCat(false)} title="New category">
        ...
        <div style={{display:"flex",alignItems:"center",gap:12,padding:14,background:nc.color+"15",borderRadius:14,marginBottom:8,border:`1px solid ${nc.color}40`}}>
          <div style={{width:48,height:48,borderRadius:14,background:nc.color+"25",display:"flex",alignItems:"center",justifyContent:"center",fontSize:24}}>{nc.icon}</div>
          <div style={{fontSize:16,fontWeight:700,color:nc.color}}>{nc.label||"Preview"}</div>
        </div>
